...
let serverInfo = JSON.parse(ajax_mcserver('/admin/mcservers/api/?function=serverInfo&id=' + serverId));

function setContentPage(pageName){
    document.getElementById('contentPageName').innerHTML = pageName;
    ajax('/admin/mcservers/api/?function=manager_page_' + pageName + '&id=' + serverId,'content',0);
}
...
function updateServerStats(){
    var stats = JSON.parse(ajax_mcserver('/admin/mcservers/api/?function=serverStats&id=' + serverId));
...
function backupServer(){
    ajax_mcserver('/admin/mcservers/api/?function=backup_server&id=' + serverId);
}
function killServer(){
    ajax_mcserver('/admin/mcservers/api/?function=kill_server_yesiamsure&id=' + serverId);
}
function sendCommand(){
    input = document.getElementById('homepage_commandInput');
    ajax_mcserver('/admin/mcservers/api/?function=sendCommand&id=' + serverId + '&command=' + encodeInString(input.value));
    input.value = "";
}
...
function modrinthContentSearch(){
    ...
    ajax('/admin/mcservers/api/?function=modrinthContentSearch&id=' + serverId + '&type=' + modrinthContentType + '&query=' + query,'modrinthContentSearchResults',0);
}
function modrinthContentLoadResultVersions(projectId,projectNameEncoded){
    ...
    ajax('/admin/mcservers/api/?function=modrinthContentLoadResultVersions&id=' + serverId + '&projectId=' + projectId + '&type=' + modrinthContentType,'modrinthContentSearchResults',0);
}
function modrinthContentApply(projectId, projectVersion){
    ajax('/admin/mcservers/api/?function=modrinthContentApply&id=' + serverId + '&projectId=' + projectId + '&projectVersion=' + projectVersion + '&type=' + modrinthContentType,'modrinthContentSearchResults',0);
    setTimeout(() => {
