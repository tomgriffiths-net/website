...
elseif($f === "serverInfo"){
    echo json_encode(communicator_client::runfunction('mcservers::serverInfo("' . $id . '");'));
}
elseif($f === "delete_server"){

    if(isset($_GET["confirm"])){
        communicator_client::runfunction('mcservers::deleteServer("' . $id . '",true);');
        header("Location: /admin/mcservers/list/");
    }
    else{
        html::head();
        ...
        echo '
            ...
            <br>
            <br>
            <a href="/admin/mcservers/list/"><button class="account-form-submit">NO!</button></a>
        ';
...
elseif($f === "create_server"){
    ...
    if(communicator_client::runfunction('mcservers::createServer(' . $datastring . ');')){
        $serverId = end(communicator_client::runfunction('mcservers::allServers();'));
        html::loadurl("/admin/mcservers/list/?range=" . $serverId . "-" . $serverId);
    }
    else{
        html::loadurl("/admin/mcservers/manager/failed-create/");
    }
}
elseif($f === "publicServer"){
    communicator_client::runfunction('mcservers::addSubdomainToServer("' . $id . '");');
}
elseif($f === "privateServer"){
    communicator_client::runfunction('mcservers::deleteSubdomainForServer("' . $id . '");');
}
...
elseif($f === "manager_page_files"){
    $dir = getServerDir($id);
    echo '<form action="/admin/mcservers/api/?function=manager_files_submit&id=' . $id . '" method="post">';
    $files = glob($dir . '*.*');
    $fileSettings = array();
...
elseif($f === "manager_files_submit"){
    ...
    html::loadurl('/admin/mcservers/manager/?setPage=files&id=' . $id);
}
...
elseif($f === "manager_page_runtime_serverproperties"){
    $propertiesSpec = communicator_client::runfunction('mcservers::serverPropertiesFileInfo()');
    $serverProperties = communicator_client::runfunction('mcservers::parseServerPropertiesFile("' . $id . '");');
    echo '<form action="/admin/mcservers/api/?function=manager_runtime_serverproperties_submit&id=' . $id . '" method="post">';
...
elseif($f === "manager_runtime_serverproperties_submit"){
    $expectedProperties = communicator_client::runfunction('mcservers::serverPropertiesFileInfo()');
    $newProperties = array();
    ...
    if(communicator_client::runfunction('mcservers::modifyServerPropertiesFile("' . $id . '",' . arrayToEvalString($newProperties) . ');')){
        html::loadurl('/admin/mcservers/manager/?setPage=runtime_serverproperties&settingsApplied=true&id=' . $id);
    }
    else{
        echo "Failed.";
    }
...
elseif($f === "manager_page_mods"){
    echo '
        ...
        <br>
    ';
    echo listContent(communicator_client::runfunction('mcservers::listContents("' . $id . '","mod")'));
}
elseif($f === "manager_page_datapacks"){
    echo '
        ...
        <br>
    ';
    echo listContent(communicator_client::runfunction('mcservers::listContents("' . $id . '","datapack")'));
}
elseif($f === "manager_page_plugins"){
    echo '
        ...
        <br>
    ';
    echo listContent(communicator_client::runfunction('mcservers::listContents("' . $id . '","plugin")'));
}
elseif($f === "manager_page_resourcepacks"){
    echo '
        ...
        <br>
    ';
    echo listContent(communicator_client::runfunction('mcservers::listContents("' . $id . '","resourcepack")'));
}
...
elseif($f === "manager_page_actions"){
    echo '
        <button onclick="window.location.href=\'/admin/mcservers/api/?function=delete_server&id=' . $id . '\';" class="btn btn-danger">Delete Server</button>
    ';
}
elseif($f === "modrinthContentSearch"){
    ...
    $comString .= ');';
    $results = communicator_client::runfunction($comString);
...
elseif($f === "modrinthContentApply"){
    $result = communicator_client::runfunction('mcservers::addModrinthContentToServer("' . $id . '", "' . $_GET['projectId'] . '", "' . $_GET['projectVersion'] . '", "' . $_GET['type'] . '");');
    if(is_string($result)){
...
function getServerDir($id){
    if(website_mcservers::validateId($id)){
        return communicator_client::runfunction('mcservers::serverDir()') . '\\' . $id . '\\';
    }
    return false;
}